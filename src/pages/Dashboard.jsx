...
function Dashboard() {
  ...
  const expenseFormMonth =
    getTransactionMonth(expenseDate);
  ...
  const effectiveExpenseCategory =
    budgetedCategories.includes(expenseCategory)
      ? expenseCategory
  ...
  const income = currencyTransactions.reduce(
    (total, transaction) => {
  ...
  const expenses = currencyTransactions.reduce(
    (total, transaction) => {
  ...
  const handleAddIncome = async () => {
    ...
    const month = getTransactionMonth(
      incomeDate,
    );
  ...
  const handleAddExpense = async () => {
    if (budgetedCategories.length === 0) {
      toast.error(
        `Create a budget for ${formatMonthLabel(
          expenseFormMonth,
        )} first.`,
      );
      return;
    ...
    if (!effectiveExpenseCategory) {
      toast.error(
        "Select a budgeted category.",
      );
      return;
    }

    const sanitizedExpenseInput = String(
      expenseInput ?? "",
    )
      .trim()
      .replace(/,/g, "");
    ...
    if (!budget) {
      toast.error(
        `${effectiveExpenseCategory} has no budget for ${formatMonthLabel(
          month,
        )}.`,
      );
      return;
    ...
    if (remaining <= 0) {
      toast.error(
        `${effectiveExpenseCategory} budget is exhausted.`,
      );
      return;
    }
  ...
  const handleUpdateTransaction =
    async () => {
      ...
      if (
        ...
        if (!budget) {
          toast.error(
            `You have not created a budget for ${category} in ${formatMonthLabel(
              updatedMonth,
            )}.`,
          );
          return;
        ...
        if (amount > remaining) {
          toast.error(
            `Only ${currencySymbol}${Math.max(
              remaining,
              0,
            ).toLocaleString()} remains in your ${category} budget.`,
          );
          return;
        }
      }
